...
Variable names must not clash with \hl{existing variable names within a shared scope, keywords, primitive data types, and functions.}

They can consist \hl{alphanumeric characters in any case (but a number cannot be the first character in a variable name) and underscores.}
...
A variable must be declared before \hl{it can be used.}
...
In this case \hl{the variable redefined within the child scope takes precedence.}

Once the child scope is exited \hl{all variables declared within the child scope can no longer be used, and if a variable is redefined within a child scope, the preexisting variable once again takes precedence upon exiting the child scope.}
...
\hl{An exit statement is used to exit from any function, terminating the program and exiting with the provided value as a code.
The provided value is truncated to 8 bits if larger.}
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{C|L|C|C}
    ...
    \hline
    \lit*{string} or \lit*{char[]} & \hl{Output a string of characters.} & \lit*{\hl{"Hello!"}} & \hl{"Hello!"} \\
    \hline
    Other Array Types & \hl{Output the address of the array.} & \hl{[0, 1, 2, 3]} & \hl{0x22150} \\  
    \hline
    \lit*{pair} & \hl{Output the address of the pair.} & \lit*{newpair(a, b)}\footnotemark[1] & \hl{0x22150} \\
    \hline
  \end{tabulary}
...
\hl{The evaluation of an expression will never alter the state of any variable
inside the program.}
...
\begin{table}
  ...
  \begin{tabular}{cclc}
    ...
    \lit*{\hl{\textbackslash t}} & \lit*{0x09} & \hl{horizontal tab} & \hl{HT}\ \\
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCL}
    ...
    \lit{len} & T\lit*{[]} & \lit*{int} & Array Length \\
      \lit{ord} & \lit*{\hl{char}} & \lit*{\hl{int}} & \lit*{\hl{Ordinal}} \\
      \lit{chr} & \lit*{\hl{int}} & \lit*{\hl{char}} & \lit*{\hl{Character}} \\
    \hline
  \end{tabulary}
...
\item The \lit{ord} operator \hl{returns the decimal ASCII ordinal that represents a given character.} \fillgap{\hl{Define/describe the }\lit{\hl{ord}} \hl{operator}}{1 mark}

\item The \lit{chr} operator \hl{returns the character that represents the given ASCII ordinal decimal.} \fillgap{\hl{Define/describe the }\lit{\hl{chr}} \hl{operator}}{1 mark} 
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCCCL}
    ...
    \lit{>} & 3 & \lit*{\hl{int|chr}} & \lit*{\hl{int|chr}} & \lit*{\hl{bool}} & Greater Than \\
    \lit{>=} & 3 & \lit*{\hl{int|chr}} & \lit*{\hl{int|chr}} & \lit*{\hl{bool}} & Greater Than or Equal \\
    \lit{<} & 3 & \lit*{\hl{int|chr}} & \lit*{\hl{int|chr}} & \lit*{\hl{bool}} & Less Than \\
    \lit{<=} & 3 & \lit*{\hl{int|chr}} & \lit*{\hl{int|chr}} & \lit*{\hl{bool}} & Less Than or Equal \\
    \lit{==} & 4 & \lit*{\hl{T}} & \lit*{\hl{T}} & \lit*{\hl{bool}} & Equality \\
    \lit{!=} & 4 & \lit*{\hl{T}} & \lit*{\hl{T}} & \lit*{\hl{bool}} & Inequality \\
...
\hl{Comments are used in the code to improve code comprehension for any potential readers.
They can be written into the code using the \texttt{\#} hash symbol followed by the desired comment.
Comments will always last until the end of the line (EOL). WACC does not support multi-line comments.
Comments are ignored by the compiler. Note that hash symbols inside string or character literals
do not constitute as comments and will be preserved by the compiler.}
